...
class TensorflowParser(Parser):
    ...
    def __init__(self, meta_file, checkpoint_file, dest_nodes, inputShape = None, in_nodes = None):
        ...
        self.tf_graph = TensorflowGraph(model)
        self.tf_graph.build()
        #for n in self.tf_graph.model.node:
            #print(n.name)
    ...
    def rename_FusedBatchNorm(self, source_node):
        ...
        nodenames = bias.name.split('/')[:-1]
        nodenames.append('moving_mean')
        mean_name = '/'.join(nodenames)
        nodenames = nodenames[:-1]
        nodenames.append('moving_variance')
        var_name = '/'.join(nodenames)
        print(mean_name, var_name)
        ...
        if self.weight_loaded:
            ...
            if mean:
                ...
                self.set_weight(source_node.name, 'mean', self.ckpt_data[mean_name])
            if var:
                self.set_weight(source_node.name, 'var', self.ckpt_data[var.name])
            else:
                self.set_weight(source_node.name, 'var', self.ckpt_data[var_name])
